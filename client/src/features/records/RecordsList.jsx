...
import { Box, Card, CardMedia, CircularProgress, Fab } from '@mui/material';
...
const RecordsList = () => {
  ...
  const fetchRecord = async () => {
    try {
      ...
      console.log('レンダリングされてレコードをGETした');
    } catch (err) {
            ...
            {records.map((obj) => (
              <Card key={obj.id} sx={{ maxWidth: 450 }}>
                ...
                <div className='record_header'>
                  <p className='record_content'>dish name：{obj.dishname}</p>
                  <p className='record_content'>comment：{obj.comment}</p>
